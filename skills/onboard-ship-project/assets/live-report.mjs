...
function validate(project, policy) {
  ...
  if (policy.schemaVersion !== 1) fail('invalid policy schemaVersion');
...
function author(item) {
  if (!item.user || typeof item.user.login !== 'string' || typeof item.author_association !== 'string') return {kind: 'unknown', login: null};
  return {kind: BOT.test(item.user.login) || item.user.type === 'Bot' ? 'bot' : 'known', login: item.user.login};
}
function commonReasons(item, policy) {
  const labels = lower(labelsOf(item)); const reasons = [];
  const actor = author(item);
  if (actor.kind === 'unknown') reasons.push('unknown-author');
  if (actor.kind === 'bot') reasons.push('bot-authored');
  if (labelsOf(item).some(label => SENSITIVE.test(label)) || SENSITIVE.test(item.title ?? '') || policy.issues.sensitiveLabels.some(label => labels.has(label.toLowerCase()))) reasons.push('security-sensitive');
  if (policy.issues.blockedLabels.some(label => labels.has(label.toLowerCase()))) reasons.push('blocked');
  return reasons;
}
function recentClaim(comments, prefix, config, now) {
  if (!config.enabled) return false;
  const cutoff = now - config.expiresAfterDays * 86_400_000;
  return comments.some(comment => typeof comment.body === 'string' && comment.body.trimStart().startsWith(prefix) && config.trustedAssociations.includes(comment.author_association) && Date.parse(comment.created_at) >= cutoff);
}
...
function issueState(repo, item, policy, now) {
  const reasons = commonReasons(item, policy); const labels = lower(labelsOf(item));
  if (policy.issues.epicLabels.some(label => labels.has(label.toLowerCase()))) reasons.push('epic');
  if (!policy.issues.allowUnlabeled && !policy.issues.readyLabels.some(label => labels.has(label.toLowerCase()))) reasons.push('untriaged');
  if (policy.claims.assignees && Array.isArray(item.assignees) && item.assignees.length > 0) reasons.push('assigned');
  if (policy.claims.implementationLabels.some(label => labels.has(label.toLowerCase()))) reasons.push('claimed');
...
function currentHeadDecision(reviews, headSha) {
  const latest = new Map();
  for (const review of reviews) {
    const login = review.user?.login;
    if (typeof login !== 'string' || review.commit_id !== headSha || !['APPROVED', 'CHANGES_REQUESTED'].includes(review.state)) continue;
    const previous = latest.get(login);
    if (!previous || Date.parse(review.submitted_at) > Date.parse(previous.submitted_at)) latest.set(login, review);
  }
  return [...latest.values()].map(review => review.state);
}
function pullState(repo, item, policy, now) {
  const reasons = commonReasons(item, policy); const labels = lower(labelsOf(item));
  if (item.draft) reasons.push('draft');
  if (policy.claims.reviewLabels.some(label => labels.has(label.toLowerCase()))) reasons.push('claimed');
  if ((item.requested_reviewers?.length ?? 0) > 0 || (item.requested_teams?.length ?? 0) > 0) reasons.push('active-review-request');
  const reviews = ghLines(`repos/${repo}/pulls/${item.number}/reviews?per_page=100`);
  const decisions = currentHeadDecision(reviews, item.head?.sha);
  ...
  if (policy.claims.comments.enabled && item.comments > 0 && recentClaim(ghLines(`repos/${repo}/issues/${item.number}/comments?per_page=100`), policy.claims.comments.reviewPrefix, policy.claims.comments, now)) reasons.push('claimed');
  return [...new Set(reasons)].sort();
}
